<template>
  <IconButton class="fixed right-2 mt-2 z-40" :icon="PlusCircleIcon" @click="isCreateModalOpen = true"/>
  <GenreGallery class="justify-center" :genres="genres" />
  <ConfirmCancelModal title="Nouveau genre" v-model:open="isCreateModalOpen" button-confirm-theme="normal" @confirm="onConfirmCreate">
    <GenreForm :has-changed="true" v-model:is-valid="isValid" v-model:genre="newGenre">
      <template #buttons="{ canSave }">
        <div class="mt-4 flex">
          <Button :disabled="!canSave" text="Confirmer" @click="onConfirmCreate"/>
          <Button theme="gray" text="Annuler" class="ml-1" @click="isCreateModalOpen = false"/>
        </div>
      </template>
    </GenreForm>
