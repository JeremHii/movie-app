<template>
  <div v-if="genre && savedGenre" class="flex flex-col">
    ...
    <div class="px-8 md:px-16 lg:px-40 py-4" v-if="genre.movies.length > 0">
      <div class="text-xl dark:text-white font-semibold">
        Films liés au genre ({{ genre.movies.length }})
      </div>
    ...
    >
      <GenreForm
        :has-changed="!_.isEqual(genre, savedGenre, { deep: true })"
...
const loadGenre = async () => {
  ...
  genre.value = await Api.Genre.getOne(parseInt(route.params.id));
  if (genre.value === null) {
...
const onConfirmEdit = async () => {
  if (!genre.value || !savedGenre.value || !isValid.value || !genre.value.id)
    return;
