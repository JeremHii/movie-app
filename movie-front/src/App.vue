<template>
  <Navbar />
  <Button v-if="canPopulate" text="Remplir la base de données" class="fixed left-2 mt-2 z-40" @click="isPopulateModalOpen = true" :icon="CircleStackIcon"/>
  <ConfirmCancelModal
  ...
  >
    <Range v-if="!isPopulating" label="Nombre de films à charger" v-model:model-value="moviesToLoad" :min="20" :max="300" :step="20" showValue/>
    <div v-if="isPopulating" class="flex justify-center items-center my-8 space-x-4">
      <Spinner/>
      <div class="dark:text-white text-sm">Chargement de {{moviesToLoad}} films dans la base de données.<br/> Cela peut prendre plusieurs minutes, ne quittez pas la page.</div>
    </div>
  </ConfirmCancelModal>
  <RouterView />
</template>
...
const onConfirmPopulate = async () => {
  const pageCount = Math.floor((typeof moviesToLoad.value === "string" ? parseInt(moviesToLoad.value) : moviesToLoad.value)/20)
